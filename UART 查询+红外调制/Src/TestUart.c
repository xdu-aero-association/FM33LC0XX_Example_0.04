...
//			LL_GPIO_Init(GPIOC, &GPIO_InitStruct);	
	...
	UART_InitStruct.BaudRate = 2400;								//波特率
	...
	UART_InitStruct.InfraredModulation = ENABLE;			       	//红外发送使能 
...
{
	LL_UART_InfraRed_InitTypeDef InfraRed_InitStruct={0};
	uint8_t TestTxData[11] = "TestUart-TX";
	uint8_t i;
	volatile uint8_t tmp08;
	
	InfraRed_InitStruct.Polarity                  = LL_UART_INFRARED_POLARITY_POSITIVE;//LL_UART_INFRARED_POLARITY_NEGATIVE;
	InfraRed_InitStruct.ModulationDuty            = 50;
	InfraRed_InitStruct.ModulationFrequency       = 38000;

	LL_UART_InfraRed_Init(UARTx, &InfraRed_InitStruct);
