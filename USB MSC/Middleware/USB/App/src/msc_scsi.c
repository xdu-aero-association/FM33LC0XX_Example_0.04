...
* @retval status
*/
int8_t SCSI_ProcessCmd(void *pdev, uint8_t lun, uint8_t *params)
{
    cdev = pdev;
...
static int8_t SCSI_ProcessRead(uint8_t lun)
{
    ...
    if (USBD_STORAGE_fops->Read(lun, MSC_BOT_Data,
        SCSI_blk_addr / SCSI_blk_size, len / SCSI_blk_size) < 0)
    {
...
static int8_t SCSI_ProcessWrite(uint8_t lun)
{
    ...
    if (USBD_STORAGE_fops->Write(lun, MSC_BOT_Data, 
        SCSI_blk_addr / SCSI_blk_size, len / SCSI_blk_size) < 0)
    {
