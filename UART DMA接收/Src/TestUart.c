...
void UART_DMA(uint8_t *buffer, uint32_t length,UART_Type* UARTx)
{
	...
	switch((uint32_t)UARTx)
	{
		case UART0_BASE:
				...
				LL_DMA_Init(DMA, &DMA_InitStruct, LL_DMA_CHANNEL_1);		
						
				LL_DMA_Enable_DMA(DMA);     
				LL_DMA_ClearFlag_Finished(DMA, LL_DMA_CHANNEL_1);
				LL_DMA_Enable_Channel(DMA, LL_DMA_CHANNEL_1);
			break;
		
		case UART1_BASE:
				DMA_InitStruct.PeriphAddress = LL_DMA_PERIPHERAL_FUNCTION3;	
				DMA_InitStruct.MemoryAddress = (uint32_t)buffer;		
				DMA_InitStruct.Direction = LL_DMA_DIR_PERIPHERAL_TO_RAM;	
				DMA_InitStruct.MemoryAddressIncMode = LL_DMA_INCREMENTAL_INCREASE;	
				DMA_InitStruct.DataSize = LL_DMA_BAND_WIDTH_BYTE;
		...
		case UART5_BASE:
				DMA_InitStruct.PeriphAddress = LL_DMA_PERIPHERAL_FUNCTION6;	
				DMA_InitStruct.MemoryAddress = (uint32_t)buffer;		
				DMA_InitStruct.Direction = LL_DMA_DIR_PERIPHERAL_TO_RAM;	
				DMA_InitStruct.MemoryAddressIncMode = LL_DMA_INCREMENTAL_INCREASE;	
				DMA_InitStruct.DataSize = LL_DMA_BAND_WIDTH_BYTE;
...
void Test_Uartx(UART_Type* UARTx)
{
	...
	UART_DMA(TestTxData,10,UARTx);
	LL_UART_EnableDirectionRx(UARTx);

	//UART0_LL_DMA_CHANNEL_1
	//UART1_LL_DMA_CHANNEL_3
	//UART4_LL_DMA_CHANNEL_2
	//UART5_LL_DMA_CHANNEL_4
	while (!LL_DMA_IsActiveFlag_Finished(DMA, LL_DMA_CHANNEL_1));
	LL_DMA_ClearFlag_Finished(DMA, LL_DMA_CHANNEL_1);
